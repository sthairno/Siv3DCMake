...
enum class GameState { Waiting, Playing, GameOver };
...
constexpr double GroundDinoCenterY() { return GroundY - 42.0 * DinoScale; }

RectF DinoHitbox(const Vec2 &center) {
  return RectF{center.x + DinoHitboxOffset.x - DinoHitboxSize.x * 0.5,
               center.y + DinoHitboxOffset.y - DinoHitboxSize.y * 0.5,
               DinoHitboxSize.x, DinoHitboxSize.y};
}
...
void DrawGround(double groundOffset) {
  ...
  for (double x = -dashSpacing + Math::Fmod(groundOffset, dashSpacing);
       x < SceneSize.x; x += dashSpacing) {
...
void DrawCactus(const Obstacle &obstacle) {
  ...
  const RectF body{obstacle.hitbox.x, obstacle.hitbox.y, obstacle.width * 0.45,
                   obstacle.height};
  body.draw(color);

  const double armW = obstacle.width * 0.35;
  const double armH = obstacle.height * 0.35;
  RectF{body.x - armW * 0.6, body.y + obstacle.height * 0.25, armW, armH}.draw(
      color);
  RectF{body.x + body.w - armW * 0.4, body.y + obstacle.height * 0.45, armW,
        armH}
      .draw(color);
}
...
double NextSpawnGap(const Game &game) {
  const double speedFactor =
      Clamp(game.scrollSpeed / InitialScrollSpeed, 1.0, 1.6);
...
bool JumpUp() { return KeySpace.up() || KeyUp.up() || MouseL.up(); }
  ...
  while (System::Update()) {
    ...
    if (game.state == GameState::Playing) {
      ...
      const double gravity =
          (!game.onGround && game.velocityY < 0 && JumpHeld())
              ? Gravity * JumpHoldGravityScale
              : Gravity;
      ...
      game.elapsed += dt;
      game.scrollSpeed =
          Min(InitialScrollSpeed + game.elapsed * SpeedAccel, MaxScrollSpeed);
      ...
      game.obstacles.remove_if([](const Obstacle &obstacle) {
        return (obstacle.hitbox.x + obstacle.hitbox.w < 0);
      });
    ...
    if (game.state == GameState::Waiting) {
      uiFont(U"Press Space to start")
          .drawAt(SceneSize.x * 0.5, 90, ColorF{0.35});
    } else if (game.state == GameState::GameOver) {
      font(U"GAME OVER").drawAt(SceneSize.x * 0.5, 90, ColorF{0.2});
      uiFont(U"Press Space to restart")
          .drawAt(SceneSize.x * 0.5, 125, ColorF{0.35});
    }
  }
}
